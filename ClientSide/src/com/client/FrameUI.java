...
public class FrameUI extends javax.swing.JFrame {
        ...
        addWindowListener(new WindowAdapter() {
            ...
            public void windowClosing(WindowEvent e) {
                try {
                    ...
                    server.removeUser(InetAddress.getLocalHost().getHostAddress());
                } catch (UnknownHostException ex) {
                    JOptionPane.showMessageDialog(null, ex.getMessage(), "Log out error", JOptionPane.ERROR_MESSAGE);
    ...
    private void initComponents() {
        ...
        menuLayer.setBackground(new java.awt.Color(0,0,0,185));
        ...
        jScrollPane2.setBounds(0, 0, 980, 370);
        ...
        emp_tabel.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Name", "Sex", "Address", "Age", "DOA", "Phone", "Salary", "WorkField"
            }
        ));
        ...
        tabbedPane.setBounds(10, 10, 980, 410);
        ...
        search_input.setBounds(120, 60, 430, 30);
        searchtab2.add(jSeparator2);
        jSeparator2.setBounds(120, 90, 430, 10);
        ...
        disp_search.setBounds(590, 50, 79, 40);
        ...
        searchtab2.add(disp_refresh);
        disp_refresh.setBounds(590, 130, 100, 40);
        ...
        searchtab2.add(deletebut);
        deletebut.setBounds(710, 130, 90, 40);
        ...
        tabbedpane2.addTab("Report", scrollpane4);

        display_Tpane.add(tabbedpane2);
        tabbedpane2.setBounds(10, 440, 980, 220);
        ...
        budget_go.setBounds(860, 530, 80, 40);
        ...
        budget_clear.setBounds(760, 530, 80, 40);
        ...
        menuback.setIcon(new javax.swing.ImageIcon(getClass().getResource("/icons/abeba.jpg"))); // NOI18N
        mainWorkspace.add(menuback);
        menuback.setBounds(0, 0, 120, 690);
...
//register new client on the server   
    ...
    public void activateClient() {
        try {
            profile = new Profile(DialogFrame.logindialog.getUserName(), DialogFrame.logindialog.getBranch(), InetAddress.getLocalHost().getHostAddress());
            server.addNewUser(profile);
        } catch (RemoteException ex) {
            JOptionPane.showMessageDialog(this, ex.getMessage(), "Remote Connect Error", JOptionPane.ERROR_MESSAGE);
        } catch (UnknownHostException ex) {
            JOptionPane.showMessageDialog(null, ex.getMessage(), "IP reading Error", JOptionPane.ERROR_MESSAGE);
        }
    }
...
//budget tab buttons
    private void budget_goActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_budget_goActionPerformed
        ...
        if (!hasEmpty(textFileds)) {
            query = "INSERT INTO budget values(" + b_field.getText().replaceAll(",", "") + "," + fund_field.getText().replaceAll(",", "") + ",'"
                    + bdate_field.getText() + "','" + DialogFrame.logindialog.getBranch() + "',0)";
            try {
                server.insertToDb(query);
                JOptionPane.showMessageDialog(this, "Insertion Succeed !!", "Success", JOptionPane.PLAIN_MESSAGE);
...
//report retreival 
    private void sale_tabelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_sale_tabelMouseClicked
        ...
        if (sale_tabel.getModel().getRowCount() > 0) {
            rowIndicator.setText("You selected row -> " + (selectedRow + 1));
    ...
    private void emp_tabelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_emp_tabelMouseClicked
        ...
        if (emp_tabel.getModel().getRowCount() > 0) {
            rowIndicator.setText("You selected row -> " + (selectedRow + 1));
            query = "SELECT Report from employee WHERE ID=" + String.valueOf((selectedRow + 1));
            try {
                map = server.selectFromDb(query);
                if (!map.isEmpty()) {
                    report_display.setText(map.get(0).get("Report").toString());
    ...
    private void purchase_tabelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_purchase_tabelMouseClicked
        ...
        if (purchase_tabel.getModel().getRowCount() > 0) {
            rowIndicator.setText("You selected row -> " + (selectedRow + 1));
            String transID = sale_tabel.getModel().getValueAt(selectedRow, 6).toString();
...
//update button handler
    private void disp_refreshActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_disp_refreshActionPerformed
        ...
        if (disp_sTpane.isVisible()) {
            ...
            TableFiller.updateSaleTable(sale_tabel, server, report_display.getText(), row);
        } else if (disp_pTpane.isVisible()) {
            row = purchase_tabel.getSelectedRow();
            if (purchase_tabel.isEditing()) {
                purchase_tabel.getCellEditor().stopCellEditing();
            }
            TableFiller.updateBuyTable(purchase_tabel, server, report_display.getText(), row);
        } else if (disp_eTpane.isVisible()) {
            row = emp_tabel.getSelectedRow();
            if (emp_tabel.isEditing()) {
                emp_tabel.getCellEditor().stopCellEditing();
            }
            TableFiller.updateEmployeeTable(emp_tabel, server, report_display.getText(), row);
        }
